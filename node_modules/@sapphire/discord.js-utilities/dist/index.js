...
class PaginatedMessage {
     ...
     */
    addPageEmbed(embed) {
        return this.addPage({ embeds: utilities.isFunction(embed) ? [embed(new discord_js.MessageEmbed())] : [embed] });
    }
    /**
    ...
    addAsyncPageEmbed(embed) {
        return this.addPage(async () => ({ embeds: utilities.isFunction(embed) ? [await embed(new discord_js.MessageEmbed())] : [embed] }));
    }
    /**
    ...
    addPageEmbeds(embeds) {
        let processedEmbeds = utilities.isFunction(embeds)
            ? embeds(new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed())
            : embeds;
    ...
    addAsyncPageEmbeds(embeds) {
        return this.addPage(async () => {
            let processedEmbeds = utilities.isFunction(embeds)
                ? await embeds(new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed(), new discord_js.MessageEmbed())
                : embeds;
            if (processedEmbeds.length > 10) {
                processedEmbeds = processedEmbeds.slice(0, 10);
            }
    ...
    async handlePageLoad(page, channel, index) {
        ...
        const resolved = options instanceof discord_js.MessagePayload ? options : new discord_js.MessagePayload(channel, this.applyTemplate(this.template, options));
...
class LazyPaginatedMessage extends PaginatedMessage {
    ...
    addPageEmbed(embed) {
        return this.addPage(() => ({ embeds: typeof embed === 'function' ? [embed(new discord_js.MessageEmbed())] : [embed] }));
    }
    addPageEmbeds(embeds) {
        return this.addPage(() => {
            let processedEmbeds = utilities.isFunction(embeds)
...
class PaginatedFieldMessageEmbed extends PaginatedMessage {
    constructor() {
        ...
        Object.defineProperty(this, "embedTemplate", {
            ...
            configurable: true,
            writable: true,
            value: new discord_js.MessageEmbed()
        });
    ...
    setTemplate(template) {
        this.embedTemplate = template instanceof discord_js.MessageEmbed ? template : new discord_js.MessageEmbed(template);
        return this;
    }
    /**
    ...
    generatePages() {
        const template = this.embedTemplate instanceof discord_js.MessageEmbed ? this.embedTemplate.toJSON() : this.embedTemplate;
        for (let i = 0; i < this.totalPages; i++) {
            const clonedTemplate = new discord_js.MessageEmbed(template);
...
const canReadMessagesPermissions = new discord_js.Permissions(['VIEW_CHANNEL']);
/**
...
const canSendMessagesPermissions = new discord_js.Permissions([canReadMessagesPermissions, 'SEND_MESSAGES']);
/**
 * Determines whether or not we can send messages in a given channel.
 * @param channel The channel to test the permissions from.
 * @returns Whether or not we can send messages in the specified channel.
...
const canSendEmbedsPermissions = new discord_js.Permissions([canSendMessagesPermissions, 'EMBED_LINKS']);
/**
...
const canSendAttachmentsPermissions = new discord_js.Permissions([canSendMessagesPermissions, 'ATTACH_FILES']);
/**
...
const canReactPermissions = new discord_js.Permissions([canSendMessagesPermissions, 'READ_MESSAGE_HISTORY', 'ADD_REACTIONS']);
/**
...
const canRemoveAllReactionsPermissions = new discord_js.Permissions([canReadMessagesPermissions, 'READ_MESSAGE_HISTORY', 'MANAGE_MESSAGES']);
/**
